...
export function registerHostBreakglassTools(server: McpServer, context: HostBreakglassContext): void {
  ...
  server.registerTool("host_system_process_kill", { title: "Stop system process", description: "Terminate exactly one Windows process by default. Set tree=true only for an explicit process-tree termination. Critical and Host Breakglass stack processes are protected.", inputSchema: { pid: pos, tree: z.boolean().default(false), approval }, annotations: writeAnnotations }, async (args) => executeTool(context, "host_system_process_kill", () => hostKillSystemProcess(context, args), { pid: args.pid, target_kind: args.tree ? "system-process-tree" : "system-process" }));
