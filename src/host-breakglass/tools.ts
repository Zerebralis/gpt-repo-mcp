...
export function registerHostBreakglassTools(server: McpServer, context: HostBreakglassContext): void {
  ...
  server.registerTool("host_read_many", { title: "Read many host files", description: "Read up to 20 approved host files in one bounded call. continue_on_error preserves independent per-file failures; inspect failed and each file before dependent work.", inputSchema: { files: z.array(z.object({ path: P, offset: z.number().int().nonnegative().optional(), length: pos.optional() }).strict()).min(1).max(20), max_total_bytes: pos.max(4 * 1024 * 1024).optional(), continue_on_error: z.boolean().optional() }, annotations: readOnlyAnnotations }, async (args) => executeTool(context, "host_read_many", () => hostReadMany(context, args)));
