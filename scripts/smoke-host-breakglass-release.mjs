...
try {
  ...
  assert(healthResponse.status === 200 && report.health.tool_count === 40, 'Core health/tool contract');
  await rpc('initialize', { protocolVersion: '2025-03-26', capabilities: {}, clientInfo: { name: 'isolated-release-smoke', version: '1' } });
  const tools = await rpc('tools/list'); assert(tools.tools.length === 40, 'MCP tool count');
